...
describe('snap support in install_on_fedora()', () => {
  describe('package manager priority', () => {
    it('should prefer dnf over snap when both have the package', () => {
      const result = runWithMocks({
        availableCommands: ['dnf', 'snap'],
        existingPackages: ['dnf:jq', 'snap:jq'],
    ...
    it('should prefer yum over snap when both have the package', () => {
      const result = runWithMocks({
        availableCommands: ['yum', 'snap'],
        existingPackages: ['yum:jq', 'snap:jq'],
        script: 'install_on_fedora jq'
      })
      expect(result.exitCode).toBe(0)
    ...
    it('should fall back to snap when dnf does not have the package', () => {
      const result = runWithMocks({
        availableCommands: ['dnf', 'snap'],
    ...
    it('should fall back to snap when yum does not have the package', () => {
      const result = runWithMocks({
        availableCommands: ['yum', 'snap'],
        existingPackages: ['snap:jq'],
        script: 'install_on_fedora jq'
    ...
    it('should prefer snap over cargo when snap has the package', () => {
      const result = runWithMocks({
        ...
        script: 'install_on_fedora ripgrep'
      })
      expect(result.exitCode).toBe(0)
      const snapIndex = result.calls.findIndex(c => c.startsWith('snap:'))
  ...
  describe('--prefer-snap flag', () => {
    it('should try snap before dnf when --prefer-snap is set', () => {
      const result = runWithMocks({
        availableCommands: ['dnf', 'snap'],
        existingPackages: ['dnf:jq', 'snap:jq'],
        script: 'install_on_fedora --prefer-snap jq'
      })
      expect(result.exitCode).toBe(0)
      const snapIndex = result.calls.findIndex(c => c.startsWith('snap:'))
      const dnfIndex = result.calls.findIndex(c => c.startsWith('dnf:'))
      expect(snapIndex).toBeGreaterThan(-1)
      expect(dnfIndex).toBe(-1)
    })

    it('should try snap before yum when --prefer-snap is set', () => {
      const result = runWithMocks({
        availableCommands: ['yum', 'snap'],
        existingPackages: ['yum:jq', 'snap:jq'],
        script: 'install_on_fedora --prefer-snap jq'
      ...
      expect(result.exitCode).toBe(0)
      const snapIndex = result.calls.findIndex(c => c.startsWith('snap:'))
      const yumIndex = result.calls.findIndex(c => c.startsWith('yum:'))
      expect(snapIndex).toBeGreaterThan(-1)
      expect(yumIndex).toBe(-1)
    })

    it('should fall back to dnf when snap does not have the package', () => {
      const result = runWithMocks({
        availableCommands: ['dnf', 'snap'],
        existingPackages: ['dnf:jq'],
        script: 'install_on_fedora --prefer-snap jq'
      })
      expect(result.exitCode).toBe(0)
    ...
    it('should accept --prefer-snap at any position', () => {
      ...
      const resultMiddle = runWithMocks({
        availableCommands: ['snap'],
        existingPackages: ['snap:jq'],
        script: 'install_on_fedora jq --prefer-snap pkg2'
      })
      ...
      const resultEnd = runWithMocks({
        availableCommands: ['snap'],
        existingPackages: ['snap:jq'],
        script: 'install_on_fedora jq pkg2 --prefer-snap'
      })
  ...
  describe('error handling with --prefer-snap', () => {
    it('should error when no package is provided with --prefer-snap', () => {
      const result = runWithMocks({
        availableCommands: ['snap'],
        existingPackages: [],
        script: 'install_on_fedora --prefer-snap'
      })
    ...
    it('should return failure when no package manager has the package', () => {
      const result = runWithMocks({
        availableCommands: ['dnf', 'snap', 'nix-env', 'cargo'],
        existingPackages: [],
        script: 'install_on_fedora --prefer-snap nonexistent-pkg'
      })
      expect(result.exitCode).not.toBe(0)
    })
  })
