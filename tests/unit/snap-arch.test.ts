...
describe('snap support in install_on_arch()', () => {
  describe('package manager priority', () => {
    it('should prefer pacman over snap when both have the package', () => {
      const result = runWithMocks({
        availableCommands: ['pacman', 'snap'],
        existingPackages: ['pacman:jq', 'snap:jq'],
    ...
    it('should prefer yay over snap when both have the package', () => {
      const result = runWithMocks({
        availableCommands: ['yay', 'snap'],
        existingPackages: ['yay:jq', 'snap:jq'],
        script: 'install_on_arch jq'
      })
      expect(result.exitCode).toBe(0)
    ...
    it('should prefer paru over snap when both have the package', () => {
      const result = runWithMocks({
        availableCommands: ['paru', 'snap'],
        existingPackages: ['paru:jq', 'snap:jq'],
        script: 'install_on_arch jq'
      })
      expect(result.exitCode).toBe(0)
      expect(result.calls.some(c => c.startsWith('paru:-S'))).toBe(true)
      expect(result.calls.some(c => c.startsWith('snap:install'))).toBe(false)
    })

    it('should fall back to snap when pacman does not have the package', () => {
      const result = runWithMocks({
        availableCommands: ['pacman', 'snap'],
    ...
    it('should fall back to snap when AUR helpers do not have the package', () => {
      const result = runWithMocks({
        availableCommands: ['pacman', 'yay', 'paru', 'snap'],
        existingPackages: ['snap:jq'],
        script: 'install_on_arch jq'
      })
      expect(result.exitCode).toBe(0)
    ...
    it('should prefer snap over cargo when snap has the package', () => {
      const result = runWithMocks({
        ...
        script: 'install_on_arch ripgrep'
      })
      expect(result.exitCode).toBe(0)
      const snapIndex = result.calls.findIndex(c => c.startsWith('snap:'))
  ...
  describe('--prefer-snap flag', () => {
    it('should try snap before pacman when --prefer-snap is set', () => {
      const result = runWithMocks({
        availableCommands: ['pacman', 'snap'],
        existingPackages: ['pacman:jq', 'snap:jq'],
        script: 'install_on_arch --prefer-snap jq'
      })
      expect(result.exitCode).toBe(0)
      const snapIndex = result.calls.findIndex(c => c.startsWith('snap:'))
      const pacmanIndex = result.calls.findIndex(c => c.startsWith('pacman:'))
      expect(snapIndex).toBeGreaterThan(-1)
      expect(pacmanIndex).toBe(-1)
    })

    it('should try snap before AUR helpers when --prefer-snap is set', () => {
      const result = runWithMocks({
        availableCommands: ['yay', 'paru', 'snap'],
        existingPackages: ['yay:jq', 'snap:jq'],
        script: 'install_on_arch --prefer-snap jq'
      })
      expect(result.exitCode).toBe(0)
      const snapIndex = result.calls.findIndex(c => c.startsWith('snap:'))
      const yayIndex = result.calls.findIndex(c => c.startsWith('yay:'))
      const paruIndex = result.calls.findIndex(c => c.startsWith('paru:'))
      expect(snapIndex).toBeGreaterThan(-1)
      expect(yayIndex).toBe(-1)
      expect(paruIndex).toBe(-1)
    })

    it('should fall back to pacman when snap does not have the package', () => {
      const result = runWithMocks({
        availableCommands: ['pacman', 'snap'],
        existingPackages: ['pacman:jq'],
        script: 'install_on_arch --prefer-snap jq'
      })
      expect(result.exitCode).toBe(0)
    ...
    it('should accept --prefer-snap at any position', () => {
      ...
      const resultMiddle = runWithMocks({
        availableCommands: ['snap'],
        existingPackages: ['snap:jq'],
        script: 'install_on_arch jq --prefer-snap pkg2'
      })
      ...
      const resultEnd = runWithMocks({
        availableCommands: ['snap'],
        existingPackages: ['snap:jq'],
        script: 'install_on_arch jq pkg2 --prefer-snap'
      })
  ...
  describe('error handling with --prefer-snap', () => {
    it('should error when no package is provided with --prefer-snap', () => {
      const result = runWithMocks({
        availableCommands: ['snap'],
        existingPackages: [],
        script: 'install_on_arch --prefer-snap'
      })
    ...
    it('should return failure when no package manager has the package', () => {
      const result = runWithMocks({
        availableCommands: ['pacman', 'snap', 'nix-env', 'cargo'],
        existingPackages: [],
        script: 'install_on_arch --prefer-snap nonexistent-pkg'
      })
      expect(result.exitCode).not.toBe(0)
    })
  })
