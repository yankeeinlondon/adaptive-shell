...
2. **`is_dark_mode`** - Return true if terminal background is dark
3. **`is_light_mode`** - Return true if terminal background is light
...
```
Tests in scope: tests/color.test.ts
Total tests: 41 (14 original + 27 new)
Passing: 41
Failing: 0
```
...
- Uses `BASH_REMATCH` for regex capture groups
- Uses `$((16#hex))` for hex-to-decimal conversion
- Uses standard utilities: `stty`, `dd`, `printf`

**Zsh:** Not compatible (intentional)
- The script header is `#!/usr/bin/env bash`
- `BASH_REMATCH` is Bash-specific (zsh uses `match` array)
- This matches existing patterns in `color.sh` which uses Bash-specific features
...
- [ ] **Caching:** Could cache the terminal background color to avoid repeated OSC queries
  - Priority: Optional
  - Reason: Most use cases call detection once at startup

- [ ] **tmux passthrough:** Could add tmux-specific escape sequence wrapping
  - Priority: Optional
  - Reason: Users can set `TERMINAL_THEME` override as workaround

- [ ] **OSC 10 support:** Could add foreground color query for contrast calculations
  - Priority: Optional
  - Reason: Out of scope for current feature

### Test Coverage: Comprehensive

- 27 new tests covering:
  - Environment variable overrides (`TERMINAL_BG_COLOR`, `TERMINAL_THEME`)
