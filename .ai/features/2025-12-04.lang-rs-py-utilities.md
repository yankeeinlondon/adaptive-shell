...
- [x] Step 2: Planning (detailed below)
- [x] Step 3: Unit tests created via tester sub-agent
- [x] Step 4: Implementation complete
- [x] Step 5: Review complete

## Final Test Results

- **lang-rs.sh**: 56 tests, all passing
- **lang-py.sh**: 94 tests, all passing
...
- Consistent structure matching `lang-js.sh` pattern
- Good use of helper functions with underscore prefix
- Proper error handling with return codes
- CLI handler for direct invocation
- Comprehensive test coverage

**Architecture:**
- Both scripts follow the established module pattern
- Source guards prevent re-execution
- Path setup handles both sourced and direct execution modes

### Potential Improvements (Optional)

- [ ] **Performance**: Consider caching `get_cargo_toml()` and `get_pyproject_toml()` results when called multiple times during dependency checks
- [ ] **Code Deduplication**: Helper functions `_check_file`, `_has_pyproject_tool`, `_has_setup_cfg_section` are duplicated between `get_py_linter_by_config()` and `get_py_formatter_by_config()` - could be extracted to module level
- [ ] **Edge Case**: The regex patterns for dependency detection could be enhanced to handle more complex TOML table formats with nested arrays

### Security

- No user input injection risks (function parameters are used safely)
- No command injection vulnerabilities
- File operations use proper quoting
- All external data is properly escaped in grep patterns

### Compatibility

- Both scripts are compatible with Bash 3.x and Bash 4+
- The `local -n` nameref feature is used (Bash 4.3+) for pass-by-reference functionality
- Tested with bash shell; zsh compatibility assumed based on similar patterns in codebase
